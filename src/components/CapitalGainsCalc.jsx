...
// ===== 2026 세율 =====
// US Long-term CGT (Single filer)
const US_LT_SINGLE = [
  { max: 49450,  rate: 0 },
  { max: 545500, rate: 0.15 },
  { max: Infinity, rate: 0.20 },
]
// US Long-term CGT (Married Filing Jointly)
const US_LT_MFJ = [
  { max: 98900,  rate: 0 },
  { max: 613700, rate: 0.15 },
  { max: Infinity, rate: 0.20 },
]
// US Short-term = ordinary income brackets (Single)
const US_ST_SINGLE = [
  { max: 11925,  rate: 0.10 },
  { max: 48475,  rate: 0.12 },
...
// UK: basic rate band threshold (2026/27)
const UK_BASIC_RATE_BAND = 50270
const UK_PERSONAL_ALLOWANCE = 12570
const UK_AEA = 3000 // Annual Exempt Amount
...
export default function CapitalGainsCalc() {
  ...
  const calculate = () => {
    ...
    const gain = sell - buy
    if (gain <= 0) {
      setResult({ gain, tax: 0, loss: true })
      return
    }
    ...
    <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button key={opt.value} onClick={() => setHoldingType(opt.value)}
                  className={`py-2 px-3 rounded-lg text-sm font-medium border transition-all ${
                    holdingType === opt.value ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                  }`}>
                  {opt.label}
                </button>
              ))}
      ...
      <div className="mb-6">
        <label className="block text-sm font-semibold text-gray-700 mb-1">
          {country === 'us' ? 'Annual Income (before this gain)' : 'Annual Income (gross)'}
        </label>
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{country === 'uk' ? '£' : '$'}</span>
        ...
        <div className="mt-6 space-y-4">
          {result.loss ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-5 text-center">
              ...
              <p className="text-green-700 font-black text-3xl mt-1">{fmt(Math.abs(result.gain), country)}</p>
            ...
            <>
              <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-5">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-4xl font-black text-indigo-700">{fmt(result.total, country)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Your Gain</p>
                    <p className="text-2xl font-bold text-gray-700">{fmt(result.gain, country)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{fmt(result.gain, country)}</span>
                </div>
                ...
                {country === 'uk' && (
                  <>
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-green-600">- {fmt(UK_AEA, country)}</span>
                    </div>
                    {result.basicPart > 0 && (
                      <div className="flex justify-between">
                        <span className="text-gray-500">Basic rate (18%)</span>
                        <span className="font-semibold">{fmt(result.basicPart * 0.18, country)}</span>
                      </div>
                    )}
                    {result.higherPart > 0 && (
                      <div className="flex justify-between">
                        <span className="text-gray-500">Higher rate (24%)</span>
                        <span className="font-semibold">{fmt(result.higherPart * 0.24, country)}</span>
                      </div>
                    )}
                  </>
                )}
                ...
                {country === 'us' && (
                  <>
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{fmt(result.tax, country)}</span>
                    </div>
                    {result.niit > 0 && (
                      <div className="flex justify-between">
                        <span className="text-gray-500">NIIT (3.8%)</span>
                        <span className="font-semibold">{fmt(result.niit, country)}</span>
                      </div>
                    )}
                  </>
                )}
                ...
                <div className="border-t border-gray-200 pt-2 flex justify-between font-bold">
                  ...
                  <span className="text-green-600">{fmt(result.gain - result.total, country)}</span>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-700 space-y-1">
                {country === 'us' ? (
                  <>
                    <p className="font-bold">ℹ️ US CGT 2026 Info</p>
                    <p>• Long-term (held {'>'}1 year): 0% / 15% / 20% based on income</p>
                    <p>• Short-term (held ≤1 year): taxed as ordinary income (10%–37%)</p>
                    <p>• NIIT 3.8% applies if income exceeds $200k (single) / $250k (MFJ)</p>
                    <p>• State taxes not included — varies by state</p>
                  </>
                ) : (
                  <>
                    <p className="font-bold">ℹ️ UK CGT 2026/27 Info</p>
                    <p>• Annual Exempt Amount: £3,000 (tax-free)</p>
                    <p>• Basic rate taxpayers: 18% on gains</p>
                    <p>• Higher/additional rate taxpayers: 24% on gains</p>
                    <p>• Basic rate band: income up to £50,270 (personal allowance £12,570)</p>
                  </>
                )}
              </div>
            </>
